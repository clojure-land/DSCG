...
str arraycopyAndInsertTuple(Argument src, Argument dst, int tupleSize, list[Argument] new, Argument newAtIndex) {
	...
	return
		"<if (isGeneric(dst)) {><toString(UNCHECKED_ANNOTATION())><}><dec(dst)> = <if (!isPrimitive(dst)) {>(<typeToString(dst.\type)>)<}> new <if (isPrimitive(dst.\type)) {><toString(asSingle(dst.\type))><} else {>Object<}>[<use(src)>.length + <tupleSize>];
...
str arraycopyAndSetTuple(Argument src, Argument dst, int tupleSize, list[Argument] new, Argument newAtIndex) {
	...
	return
		"<if (isGeneric(dst)) {><toString(UNCHECKED_ANNOTATION())><}><dec(dst)> = <if (!isPrimitive(dst)) {>(<typeToString(dst.\type)>)<}> new <if (isPrimitive(dst.\type)) {><toString(asSingle(dst.\type))><} else {>Object<}>[<use(src)>.length];
